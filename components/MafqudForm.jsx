...
export default function MafqudForm({ selectedHeirs, onCalculate, isLoading }) {
    ...
    const handleSubmit = (e) => {
        ...
        // --- TAMBAHKAN VALIDASI ---
        if (!mafqudKey) { setError('Pilih siapa ahli waris yang hilang (Mafqud).'); return; }
    ...
    return (
        <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow-md space-y-6">
            <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Pilih Ahli Waris yang Hilang (Mafqud)</label>
                <select 
                    value={mafqudKey} 
            ...
            <button 
                type="submit" 
                disabled={isLoading} 
                className="w-full py-3 rounded-lg font-bold text-lg text-white bg-green-600 hover:bg-green-700 disabled:bg-gray-400"
            >
                {isLoading ? 'Menghitung...' : 'Hitung Warisan Mafqud'}
            </button>
        </form>
    );
}