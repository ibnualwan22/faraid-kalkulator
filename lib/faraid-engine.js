...
export function calculateFaraid(input) {
    ...
    // LANGKAH 1: TENTUKAN BAGIAN & ALASAN
    let perhitunganBerhak = [];
    let penyebut = [];
    hasil.langkah_perhitungan.push({ langkah: 1, deskripsi: "Menganalisis Status Setiap Ahli Waris (Berhak atau Mahjub)", detail: [] });
    hasil.output = state.ahliWaris.map(key => ({ key, nama: ahliWarisData[key]?.name, namaAr: ahliWarisData[key]?.nameAr }));
        ...
        for (const cond of data.conditions) {
            if (cond.check(state)) {
                ...
                if (cond.bagian === 'mahjub') {
                    aw.isMahjub = true;
                    hasil.langkah_perhitungan[0].detail.push(`- ${data.name}: Mahjub karena ${cond.syarat}`);
                } else {
                    aw.isMahjub = false;
                    ...
                    hasil.langkah_perhitungan[0].detail.push(`- ${data.name}: Berhak ${cond.bagian} karena ${cond.syarat}`);
                }
                break;
            }
        }
    ...
    // BLOK 'ASHABAH MURNI' YANG SUDAH DIPERBAIKI
    if (dzawilFurudh.length === 0 && ashabahGroup.length > 0) {
        hasil.langkah_perhitungan.push({ langkah: 1.1, deskripsi: "Kasus Ashabah Murni Terdeteksi", detail: ["Semua ahli waris yang berhak adalah Ashabah."] });
        
        const hasMale = perhitunganBerhak.some(aw => ahliWarisData[aw.key].gender === 'L');
        const hasFemale = perhitunganBerhak.some(aw => ahliWarisData[aw.key].gender === 'P');
        const isMixedGenderGroup = hasMale && hasFemale;

        let totalKepala = 0;
        perhitunganBerhak.forEach(aw => {
            const data = ahliWarisData[aw.key];
            const jumlah = input.ahliWaris[aw.key];
            let kepalaPerOrang = isMixedGenderGroup ? (data.gender === 'P' ? 1 : 2) : 1;
            aw.kepala = kepalaPerOrang * jumlah;
            totalKepala += aw.kepala;
        });

        const ashlulMasalah = totalKepala;
        const sahamMap = new Map();
        perhitunganBerhak.forEach(aw => {
            const bagianHarta = (input.tirkah / ashlulMasalah) * aw.kepala;
            sahamMap.set(aw.key, { sahamAwal: aw.kepala, sahamAkhir: aw.kepala, sahamTashih: aw.kepala, bagianHarta: bagianHarta, persentase: (aw.kepala / ashlulMasalah) * 100 });
        });
        ...
        hasil.summary = { input, ashlulMasalahAwal: ashlulMasalah, ashlulMasalahAkhir: ashlulMasalah, ashlulMasalahTashih: ashlulMasalah, totalSahamAwal: ashlulMasalah, kasus: "'Ashabah Murni" };
        
        const logTirkah = { langkah: 2, deskripsi: "Rincian Final Pembagian Harta (Tirkah)", detail: [] };
    ...
    // =================================================================
    // LANGKAH 7: RUMUS AKHIR & FINALISASI OUTPUT
    ...
    hasil.langkah_perhitungan.push({ langkah: 7, deskripsi: "Rincian Final Pembagian Harta (Tirkah)", detail: [] });
    const tirkah = input.tirkah || 0;

    // PERBAIKAN UNTUK MENGHINDARI REDECLARE 'sahamMap'
    const finalDataMap = new Map(perhitunganBerhak.map(i => [i.key, i]));
    hasil.output.forEach(aw => {
        if (finalDataMap.has(aw.key)) {
            Object.assign(aw, finalDataMap.get(aw.key));
        }
        ...
        if (!aw.isMahjub) {
            ...
            if (tirkah > 0) {
                 hasil.langkah_perhitungan[hasil.langkah_perhitungan.length - 1].detail.push(`- ${aw.nama}: (${sahamFinal} × Rp ${tirkah.toLocaleString('id-ID')}) / ${amFinal} = Rp ${bagianHarta.toLocaleString('id-ID', { maximumFractionDigits: 0 })}`);
            }
        } else {
            aw.bagianHarta = 0;
            aw.persentase = 0;
