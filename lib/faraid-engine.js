...
export function calculateFaraid(input) {
    ...
    if (dzawilFurudh.length === 0 && ashabahGroup.length > 0) {
        ...
        // Alih-alih mengganti hasil.output, kita PERBARUI yang sudah ada
        const sahamMap = new Map();
        perhitunganBerhak.forEach(aw => {
            const bagianHarta = (input.tirkah / ashlulMasalah) * aw.kepala;
            sahamMap.set(aw.key, {
                sahamAwal: aw.kepala,
                sahamAkhir: aw.kepala,
                sahamTashih: aw.kepala, // Untuk konsistensi
                bagianHarta: bagianHarta,
                persentase: (aw.kepala / ashlulMasalah) * 100
            });
        });
        ...
        hasil.summary = {
            input, // Simpan input asli untuk referensi
            ashlulMasalahAwal: ashlulMasalah,
            ashlulMasalahAkhir: ashlulMasalah,
            ashlulMasalahTashih: ashlulMasalah,
        ...
        // Sekarang filter aman karena semua item di hasil.output punya property isMahjub
