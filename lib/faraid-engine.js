...
import { handleJaddWalIkhwah, isAkdariyah } from './cases/jadd-wal-ikhwah.js';
...
export function calculateFaraid(input) {
    ...
    if (state.isGharawain) {
        ...
        const hasilJadd = handleJaddWalIkhwah(state, perhitunganBerhak, ashlulMasalahAwal);
        hasil.langkah_perhitungan.push({
            langkah: 3.1,
    ...
    // LANGKAH 7: RINCIAN PEMBAGIAN HARTA
    const amFinal = hasil.summary.ashlulMasalahTashih || hasil.summary.ashlulMasalahAkhir;
    const sahamFinalKey = hasil.summary.ashlulMasalahTashih ? 'sahamTashih' : 'sahamAkhir';
    
    hasil.langkah_perhitungan.push({
        langkah: 7,
        deskripsi: "Rincian Pembagian Harta (Tirkah)",
        detail: []
    });

    hasil.output.forEach(aw => {
        const sahamFinal = aw[sahamFinalKey] || 0;
        const bagianHarta = (input.tirkah / amFinal) * sahamFinal;
        aw.bagianHarta = bagianHarta;
        aw.persentase = (sahamFinal / amFinal) * 100;

        hasil.langkah_perhitungan[hasil.langkah_perhitungan.length - 1].detail.push(
            `- ${aw.nama}: (${sahamFinal} × ${input.tirkah.toLocaleString('id-ID')}) / ${amFinal} = Rp ${bagianHarta.toLocaleString('id-ID', { maximumFractionDigits: 0 })}`
        );
    });
